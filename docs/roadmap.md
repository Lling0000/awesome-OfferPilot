...
- Pasted JD parser fixtures cover backend, frontend, data, and product-style roles with JSONL metadata, including mixed Chinese/English fields and `private_user_context` boundaries.
...
- Source-backed report fixtures include JSONL metadata for retained `sourceUrls`, required claim `sourceIds`, expected evidence quality labels, and weak-source handling.
...
- Daily intelligence fixtures include JSONL metadata for company-scale labels, signal types, source types, relevance expectations, and retained `sourceUrls`.
